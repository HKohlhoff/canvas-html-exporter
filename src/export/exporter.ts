...
async function prepareNode(ctx: MarkdownContext, node: CanvasNode): Promise<CanvasNode> {
  ...
  if (ext === "md") {
    ...
    try {
      ...
      console.error(`[canvas2html] Markdown page export failed for ${file.path}`, error);
    }
    ...
    try {
      ...
      console.error(`[canvas2html] Markdown preview generation failed for ${file.path}`, error);
    }
...
<head>
  ...
  <meta name="canvas2html-build" content="${EXPORTER_VERSION}-${ctx.highlightingTheme || "shiki"}">
  ...
  for (const match of embedMatches) {
    ...
    if (!targetFile) {
      ...
      try {
        ...
        console.error(`[canvas2html] Markdown embed export failed for ${targetFile.path}`, error);
        replacement = `<span class="unresolved-link">Unresolved embed: ${escapeHtmlAttr(target)}</span>`;
      }
...
async function buildMarkdownPreview(ctx: MarkdownContext, file: TFile): Promise<{ text: string; html: string }> {
  ...
  try {
    ...
    console.error(`[canvas2html] Preview rendering failed for ${file.path}`, error);
...
<head>
  ...
  <meta name="generator" content="${EXPORTER_SIGNATURE}">
  <meta name="canvas2html-build" content="${EXPORTER_VERSION}-${highlightingTheme || "shiki"}">
